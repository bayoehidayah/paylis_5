    ...
    class Auth extends Config{
        ...
        public function login_action($username, $password, $sebagai){
            ...
            if($sebagai == "Bank" || $sebagai == "Admin"){
                $check_data = mysql_query("SELECT * FROM admin JOIN level ON admin.id_level=level.id_level WHERE admin.username='$username'");
                if(mysql_num_rows($check_data) > 0){
                    $data_user = mysql_fetch_array($check_data);
            ...
            //Lalu jika tidak cocok lanjut ke pelanggan
                ...
                $check_data = mysql_query("SELECT * FROM pelanggan WHERE username='$username'");
                if(mysql_num_rows($check_data) > 0){
                    $data_user = mysql_fetch_array($check_data);
