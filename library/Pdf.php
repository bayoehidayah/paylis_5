...
*/
    ...
    class Pdf extends Config{
        ...
        public function struk_pembayaran($id_pembayaran){
            $get_data = mysql_query("SELECT * FROM pembayaran WHERE id_pembayaran='$id_pembayaran'");

            if(mysql_num_rows($get_data) > 0){
                $pembayaran = mysql_fetch_array($get_data);
                ...
                $data_penggunaan = mysql_query("SELECT * FROM penggunaan WHERE id_penggunaan='$id_penggunaan'");
                $penggunaan = mysql_fetch_array($data_penggunaan);
